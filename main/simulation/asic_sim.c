...
static uint32_t simulated_total_counters[MAX_SIM_CHIPS] = {0};
static uint32_t simulated_domain_counters[MAX_SIM_CHIPS] = {0};
static uint32_t simulated_error_counters[MAX_SIM_CHIPS] = {0};
...
uint8_t ASIC_init(GlobalState * GLOBAL_STATE) {
    ...
    if (simulated_chip_count > MAX_SIM_CHIPS) {
        ESP_LOGW(TAG, "ASIC SIMULATION: chip count %d exceeds MAX_SIM_CHIPS %d, clamping", simulated_chip_count, MAX_SIM_CHIPS);
        simulated_chip_count = MAX_SIM_CHIPS;
    }
...
task_result * ASIC_process_work(GlobalState * GLOBAL_STATE) {
    ...
    task_result *return_val = NULL;
    if (has_pending_result) {
        has_pending_result = false;
        return_val = &simulated_result;
    }
...
void ASIC_send_work(GlobalState * GLOBAL_STATE, void * next_job) {
    ...
    if (GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[simulated_job_id_counter] != NULL) {
        free_bm_job(GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[simulated_job_id_counter]);
    }
...
void ASIC_set_frequency(GlobalState * GLOBAL_STATE) {
    ...
    GLOBAL_STATE->POWER_MANAGEMENT_MODULE.actual_frequency = target_frequency;
    ESP_LOGI(TAG, "ASIC SIMULATION: Set frequency to %.1f MHz", target_frequency);
}

/**
 * @brief Set ASIC nonce space (no-op in simulation)
 */
void ASIC_set_nonce_space(GlobalState * GLOBAL_STATE) {
    ESP_LOGD(TAG, "ASIC SIMULATION: set_nonce_space (no-op)");
}
...
void ASIC_read_registers(GlobalState * GLOBAL_STATE) {
    ...
    for (uint8_t chip = 0; chip < simulated_chip_count; chip++) {
        ...
        hashrate_monitor_register_read(GLOBAL_STATE, REGISTER_TOTAL_COUNT, chip, simulated_total_counters[chip], now_ms * 1000);
        
        // Domain 0 count register (matching real update_hashrate behavior)
        hashrate_monitor_register_read(GLOBAL_STATE, REGISTER_DOMAIN_0_COUNT, chip, simulated_domain_counters[chip], now_ms * 1000);
        
        // Error count register
        hashrate_monitor_register_read(GLOBAL_STATE, REGISTER_ERROR_COUNT, chip, simulated_error_counters[chip], now_ms * 1000);
        ...
        hashrate_monitor_register_read(GLOBAL_STATE, REGISTER_HASHRATE, chip, hr_value, now_ms * 1000);
    }
