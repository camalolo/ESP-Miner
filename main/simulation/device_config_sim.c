...
void device_config_sim_init(void) {
    ...
    if (nvs_config_get_bool(NVS_CONFIG_EMC2101) == 0) {
        nvs_config_set_bool(NVS_CONFIG_EMC2101, 1);
        ESP_LOGI(TAG, "Enabled EMC2101 for simulation");
    }
    if (nvs_config_get_bool(NVS_CONFIG_EMC2103) == 0) {
        nvs_config_set_bool(NVS_CONFIG_EMC2103, 0);  // Disable EMC2103 for Supra
        ESP_LOGI(TAG, "Disabled EMC2103 for simulation");
    }
    if (nvs_config_get_bool(NVS_CONFIG_INA260) == 0) {
        nvs_config_set_bool(NVS_CONFIG_INA260, 1);
        ESP_LOGI(TAG, "Enabled INA260 for simulation");
    }
    if (nvs_config_get_bool(NVS_CONFIG_TPS546) == 0) {
        nvs_config_set_bool(NVS_CONFIG_TPS546, 1);  // Enable TPS546 for simulation
        ESP_LOGI(TAG, "Enabled TPS546 for simulation");
    }
    if (nvs_config_get_bool(NVS_CONFIG_DS4432U) == 0) {
        nvs_config_set_bool(NVS_CONFIG_DS4432U, 1);
