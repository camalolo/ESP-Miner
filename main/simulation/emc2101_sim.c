...
static int simulated_temp_offset = 0;
...
esp_err_t EMC2101_init(int temp_offset_param) {
    ESP_LOGI(TAG, "EMC2101 SIMULATION: Initializing simulated temperature sensor (offset=%d)", temp_offset_param);

    simulated_temp_offset = temp_offset_param;
...
esp_err_t EMC2101_set_fan_speed(float percent) {
    ...
    simulated_fan_speed = (uint16_t)(500 + percent * 2000);
...
float EMC2101_get_external_temp(void) {
    ...
    float current_temp = simulated_external_temp + variation + simulated_temp_offset;
