...
float INA260_read_power(void) {
    // Read current values without causing double drift (don't call read_current/read_voltage)
    float current = simulated_current + ((float)rand() / RAND_MAX - 0.5f) * 200.0f;
    float voltage = simulated_voltage + ((float)rand() / RAND_MAX - 0.5f) * 100.0f;
    ...
    ESP_LOGD(TAG, "INA260 SIMULATION: Returning power %.3fmW (V:%.3fmV, I:%.3fmA)", power, voltage, current);
    return power;
}