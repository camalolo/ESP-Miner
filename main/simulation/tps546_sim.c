...
esp_err_t TPS546_check_status(GlobalState * GLOBAL_STATE) {
    // Faults are transient in simulation: existing fault has a chance to self-clear,
    // and new faults are extremely rare (0.01% chance).
    if (simulated_status_word != 0) {
        // 90% chance to self-clear an existing fault each check
        if ((rand() % 100) < 90) {
            simulated_status_word = 0;
            strcpy(simulated_error_message, "No faults detected");
        }
    } else if ((rand() % 10000) < 1) { // 0.01% chance of a new fault
        // Only set one specific fault bit for realism
        uint16_t fault_bits[] = {TPS546_STATUS_TEMP, TPS546_STATUS_IOUT_OC, TPS546_STATUS_VOUT_OV};
        simulated_status_word = fault_bits[rand() % 3];
        if (simulated_status_word & TPS546_STATUS_TEMP) {
            ...
            strcpy(simulated_error_message, "Simulated overvoltage fault");
        }
    }
...
void TPS546_print_status(void) {
    ESP_LOGI(TAG, "TPS546 SIMULATION: Status word: 0x%04X, %s", simulated_status_word, simulated_error_message);
    ESP_LOGI(TAG, "  Vin: %.2fV, Vout: %.3fV, Iout: %.2fA, Temp: %d°C",
             simulated_vin, simulated_vout, simulated_iout, simulated_temperature);
}

void TPS546_log_snapshot(const TPS546_StatusSnapshot *s) {
    ESP_LOGI(TAG, "TPS546 SIMULATION: Snapshot - status: 0x%04X, Vin: %.2fV, Vout: %.3fV, Iout: %.2fA",
             s->status_word, s->read_vin, s->read_vout, s->read_iout);
}

esp_err_t TPS546_snapshot_status(TPS546_StatusSnapshot *s) {
    if (s == NULL) return ESP_ERR_INVALID_ARG;
    memset(s, 0, sizeof(*s));
    s->status_word = simulated_status_word;
    s->read_vin = TPS546_get_vin();
    s->read_vout = TPS546_get_vout();
    s->read_iout = TPS546_get_iout();
    s->read_temp1 = TPS546_get_temperature();
    s->vout_command = simulated_vout;
    return ESP_OK;
}